 ...
 * The netCDF class does not distinguish <cite>Coordinate Operation</cite> from <cite>Math Transform</cite>,
 * so we implement the two interfaces by the same class.
 *
 * @author  Martin Desruisseaux (Geomatys)
 */
final class NetcdfProjection extends NetcdfIdentifiedObject
        implements org.opengis.referencing.operation.Projection, MathTransform2D
{
    ...
    /**
     * The operation method that provider this projection instance, or {@code null} if unknown.
     * This information is set by the {@link NetcdfTransformFactory} only.
     */
     ...
     * @param provider    for {@link ProjectionProvider#createProjection()} usage only.
     ...
     * The default implementation tests if the source and target CRS are equals.
     *
     ...
     * If {@code ptDst} is {@code null}, a new {@link DirectPosition} object is allocated and
     ...
     *
     * <ul>
     *   <li>The source array and the target array are the same array
     *       (note that it can never be the case if the arrays are not of the same type)</li>
    ...
    /**
     * Transforms an arbitrary amount of points from the given source array to the given destination array.
     ...
     */
    private SimpleParameterGroup getParameters() {
    ...
    /**
     * Returns a hash code value for this projection.
     */
    @Override
    public int hashCode() {
        return super.hashCode() + 41 * Objects.hashCode(sourceCRS) + 37 * Objects.hashCode(targetCRS);
    }
